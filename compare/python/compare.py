...
def compare_dataframes(src_df, tgt_df, output_file, primary_key):
    try:
        ...
        src_df1 = pd.read_csv(src_df, header=0)
        df1 = src_df1.reindex(columns=sorted(src_df1.columns))
        source_shape = f"{src_df1.shape[0]} rows and {src_df1.shape[1]} columns"

        print("Reading target file...")
        src_df2 = pd.read_csv(tgt_df, header=0)
        df2 = src_df2.reindex(columns=sorted(src_df2.columns))
        target_shape = f"{src_df2.shape[0]} rows and {src_df2.shape[1]} columns"

        # Remove duplicates from each dataset for clean comparison
        df1.drop_duplicates(inplace=True)
        df2.drop_duplicates(inplace=True)
        print("Removed duplicates from both datasets.")
        ...
        print("Standardized column names and data types.")
...
if __name__ == "__main__":
    # Example usage
    src_df = r"C:\Vikas_Drive\Projects\ETL_Data_Validation_Framework\Test_Data\source_file.csv"
    tgt_df = r"C:\Vikas_Drive\Projects\ETL_Data_Validation_Framework\Test_Data\target_file.csv"
    output_file = "C:\Vikas_Drive\Projects\ETL_Data_Validation_Framework\Test_Data\comparison_output.xlsx"
    primary_key = ["Name","Age"]  # Example primary key column(s)

    compare_dataframes(src_df, tgt_df, output_file, primary_key)
